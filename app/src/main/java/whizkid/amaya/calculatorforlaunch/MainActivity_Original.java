...
public class MainActivity_Original extends Activity {
...
//        btn.setTypeface(typeface);
    ...
    private String evaluate(String problem) {
        ...
        String result = "";
        String[] mySplits = Utils.splitMyString(problem);

        if (mySplits == null || mySplits.length == 0) {
            return "0";
        }

        if (mySplits.length == 1) {
            operand1 = mySplits[0];
            operand2 = mySplits[0];
        } else {
            operand1 = mySplits[0];
        ...
        if (problem.indexOf(Utils.ADD) > 0) {
            operator = Utils.ADD;
        } else if (problem.indexOf(Utils.SUBTRACT) > 0) {
            operator = Utils.SUBTRACT;
        } else if (problem.indexOf(Utils.MULTIPLY) > 0) {
            operator = Utils.MULTIPLY;
        } else if (problem.indexOf(Utils.DIVIDE) > 0) {
            operator = Utils.DIVIDE;
        }
        ...
        switch (operator) {
            case Utils.ADD:
                result = Double.toString(opr1 + opr2);
                break;
            case Utils.SUBTRACT:
                result = Double.toString(opr1 - opr2);
                break;
    ...
    private void setValueToTextResult(String strValue) {

        //remove starting 0. 07 -> 7
        if (strValue != null && strValue.length() > 1 && strValue.startsWith("0")) {
            strValue = strValue.substring(1);
        }

        //remove trailing operators 78++ -> 78+
        if (strValue != null && strValue.length() > 2
                && Utils.containsAnyTagAlready(strValue.substring(0, strValue.length() - 1), "+-*/")
                && Utils.isTagOperator(Character.toString(strValue.charAt(strValue.length() - 1)))) {
            strValue = strValue.substring(0, strValue.length() - 1);
        }

        textViewResult.setText(strValue);
    }
